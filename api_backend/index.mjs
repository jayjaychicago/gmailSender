// index.mjs
      ...
      const corsOptions = {
        ...
        allowedHeaders: ['Content-Type', 'Accept', 'Authorization', 'X-Amz-Date', 'x-admin-api-key',  'X-Amz-Security-Token'],
        credentials: true
      };
...
if (req.path.startsWith('/prod') || req.path.startsWith('/dev') || req.path.startsWith('/staging') || req.path.startsWith('/test') ) {
    console.log(`Normalizing path2: ${req.path} -> ${req.path.replace(/^\/(?:prod|dev|staging|test)\/[^\/]+/, '')}`);
    req.url = req.url.replace(/^\/(?:prod|dev|staging|test)\/[^\/]+/, '');
  }
...
app.get('/calendars/:calendarId/events', (req, res) => {
  ...
  res.json({
    message: `I have received your GET for /calendars/{calendarId}/events and now all you have to do is add code here to process it`,
    method: 'GET',
